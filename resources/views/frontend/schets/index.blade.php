        ...
        <div class="col-md-12">
            @can('schet_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.schets.create') }}">
                            ...
                            <tbody>
                                @foreach($schets as $key => $schet)
                                    <tr data-entry-id="{{ $schet->id }}">
                                        ...
                                        <td>
                                            @can('schet_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.schets.show', $schet->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('schet_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.schets.edit', $schet->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('schet_delete')
                                                <form action="{{ route('frontend.schets.destroy', $schet->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('schet_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.schets.massDestroy') }}",
