...
<div class="content">
    @can('notarization_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.notarizations.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $notarization->id }}">
                                        ...
                                        <td>
                                            @can('notarization_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.notarizations.show', $notarization->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('notarization_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.notarizations.edit', $notarization->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('notarization_delete')
                                                <form action="{{ route('admin.notarizations.destroy', $notarization->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('notarization_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.notarizations.massDestroy') }}",
